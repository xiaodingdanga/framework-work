		...
		<div class="box box-default">
			<div class="box-header with-border">
          		...
          		<div class="box-tools pull-right">
          			<button type="button" class="btn btn-success btn-xs" onclick="add();"><i class="fa fa-plus"></i>添加区域</button>
          		</div>
        	</div>
...
function init(parentId,height){
    ...
    jQuery("#table").jqGrid({
	    ...
	    height:height,
   	    autowidth:true,
        shrinkToFit:true,
        colNames:['id','区域名称','parentId','操作'],//列名
        colModel:[//列对应的字段
            {name:'id',index:'id',width:30,hidden:true,sortable:false},//把pid用id的形式表示出来，以便getDataIDs获取到pid
        	{name:'name',index:'name',width:60,sortable:false},   
        	{name:'parentId',index:'parentId',hidden:true,width:60,sortable:false},  
         	{name:'Edit',index:'Edit',width:60}
        ],
     	multiselect:false,//是否支持多选   true
...
var zTree;
//起始节点初始化
var zNodes=[{id:"0",name:"区域管理",isParent:true}];
var setting={
	//设置异步获取方式
	async:{
 		enable:true,
...
// 	alert(parentId);
	...
	dialog.Height = 300;
	dialog.Title="添加区域";
	dialog.URL="<%=ctx%>/city/toadd?pid="+parentId;
...
function edit(id){
	...
	dialog.Height = 300;
	dialog.Title="修改区域";
	dialog.URL="<%=ctx%>/city/toedit?id="+id;
	dialog.OKEvent=function(){
		var ret=dialog.innerFrame.contentWindow.save();
		if(ret=="ok"){
			var newpid=dialog.innerFrame.contentWindow.backpid();
