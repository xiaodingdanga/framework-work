...
<div class="row">
	<div class="col-md-12">
		<div class="box box-default">
			<div class="box-body">
				<div class="row">
					<label class="col-xs-1 control-label" style="padding: 10px 0px 0px 20px;">分销商:</label>
...
function getSum(startdate,enddate,dname,paymentid,status,customerid){
	$.ajax({
		type:'post',
		url:'<%=ctx%>/report/findAdvanceRecordSum',
		data:{
			'startdate':startdate,
			'enddate':enddate,
			'dname':dname,
...
function init(height){
    ...
    jQuery("#table").jqGrid({ 
		...
		url:'<%=ctx%>/report/findAdvanceRecord',//这是Action的请求地址 
		postData:{'startdate':'','enddate':'','dname':'','customerid':'${user.customerid}','paymentid':'','status':''},  
	    mtype:'POST',   
	    height:height,
   	    autowidth:true,
        shrinkToFit:true,
        colNames:['id','分销商','交易类型','票名称','数量','单价(元)','金额(元)','支付方式','操作员','时间'],//列名
        colModel:[//列对应的字段
            {name:'id',index:'id',width:30,hidden:true,sortable:false},
            {name:'dname',index:'dname',width:80,sortable:true},
            {name:'status',index:'status',width:30,sortable:false,formatter:"select",cellattr:addCellAttr,editoptions:{value:"-2:退卡(退押金);-1:退票;1:售票;2:售卡(收押金);3:商品销售"}},
            {name:'ticketname',index:'ticketname',width:60,sortable:false}, 
            {name:'num',index:'num',width:60,sortable:false,cellattr:addCellAttr}, 
            {name:'price',index:'price',width:80,sortable:false,cellattr:addCellAttr}, 
            {name:'amount',index:'amount',width:80,sortable:false,cellattr:addCellAttr},
            {name:'pname',index:'paymentid',width:80,sortable:false},
...
function query(){
	...
	var postdata={'startdate':startdate,'enddate':enddate,'dname':dname,'customerid':'${user.customerid}','paymentid':paymentid,'status':status};  
    $("#table").jqGrid("setGridParam",{datatype:'json',postData:postdata}).trigger("reloadGrid");
    getSum(startdate,enddate,dname,paymentid,status,'${user.customerid}');
}
//重置
function rest(){
	$("#startdate").val('');
...
function addCellAttr(rowId, val, rawObject, cm, rdata) {
	if(rawObject.status == '-1'||rawObject.status == '-2'){
...
function exports(){
	...
	var paymentid=$("#paymentid").val();
	var status=$("#status").val();
	window.location.href="<%=ctx%>/report/advanceRecordExport.action?startdate="
				+ startdate
				+ "&enddate="
				+ enddate
				+ "&dname="
				...
				+ "&paymentid="
				+ paymentid
				+ "&customerid=${user.customerid}&status=" + status;
	}
</script>